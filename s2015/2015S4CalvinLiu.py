...
def main():  # Put everything into a function speeds it up a little bit
    k, n, m = list(map(int, file.readline().split()))
    graph = [[] for i in range(n + 1)]  # Make adjacency list
    for i in range(m):
    ...
    s, t = list(map(int, file.readline().split()))
    dp = [[1555555555 for j in range(k + 1)] for i in
          range(n + 1)]  # A 2D array called dp (the idea is dp-ish and the name dp is short) to store the answers
    for i in range(k + 1):
        dp[s][i] = 0  # The distance from the source to the source is definitely 0
    flag = [False for i in range(
        n + 1)]  # A flag to prevent pushing the same node onto the queue when the node is already in the queue
